class Place {
	...
	show_place(cplace) {
		...
		if (cplace.socialMediaUserPlace != null && cplace.socialMediaUserPlace.length != 0) {
			...
			this.social_media_img.attr("src", "./img/" + socialmedia[0] + ".svg");
		...
		document.getElementById("data_place").scrollTo(0, document.getElementById("data_place").scrollHeight);
	}
	...
	map_marker_click(locations, id, isMarker, index) {
		...
		if (cplace) {
			...
			if (page == "map") {
				...
				window.history.replaceState({}, "", "/web/acolmenero/photowhere/map.html?outid="+id);
				// window.history.replaceState({}, "", "/photowhere/map.html?outid="+id);
